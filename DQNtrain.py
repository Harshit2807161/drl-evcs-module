...
dist = 'N'
for j in NO_OF_CARS:
    model_dir = f"models/DQN/{dist}/{j}/{int(time.time())}"
    log_dir = f"logs/DQN/{dist}/{j}/{int(time.time())}"
    ...
    model = DQN("MlpPolicy", env, verbose=1, tau=0.4, double_dqn=True, learning_rate=8e-3, learning_starts=30, exploration_fraction=0.7,  batch_size=128, tensorboard_log=log_dir, device="cuda", buffer_size=100000) 
    no_episodes = 2000
    TIMESTEPS = (j)*no_episodes
    print("Number of timesteps: ",TIMESTEPS)
    
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False,tb_log_name="DQN",progress_bar="True")
    model.save(f"{model_dir}/{TIMESTEPS}")
    
    save_dir = f"results/DQN/{dist}/{j} cars/"
