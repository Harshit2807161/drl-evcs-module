...
dist = DISTS[1]
for j in NO_OF_CARS:
    model_dir = f"models/PPO_oneagent/{dist}/{j}/{int(time.time())}"
    log_dir = f"logs/PPO_oneagent/{dist}/{j}/{int(time.time())}"
    ...
    model = PPO("MlpPolicy", env, verbose=1,n_steps=2048, tensorboard_log=log_dir, batch_size=64, device="cuda") 
    no_episodes = 2000
    ...
    save_dir = f"results/PPO/{dist}/{j} cars/"
